...
<sql:query var ="projects" dataSource="jdbc/company">
    ...
    <c:forEach var ="dpt" items ="${departments.rows}" varStatus ="iter">
        <tr>
            <td>${dpt.dname}</td>
            <td><a href="empView?ssn=${dpt.mgrssn}">${dpt.lname},${dpt.fname}</a></td>
            <td>${dpt.mgrstartdate}</td>
        </tr>
    </c:forEach>
    </table>
    ...
    <c:forEach var = "loc" items = "${location.rows}">
        <tr>
            <td>Location</td>
        </tr>
    </c:forEach>
