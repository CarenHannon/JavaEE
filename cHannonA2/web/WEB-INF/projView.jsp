...
<sql:query var="employees" dataSource="jdbc/company">
    ...
    <table>
        ...
        <c:forEach var="proj" items="${selectedProject}">
            <tr>
                <td>${proj.pname}</td>
                <td>${proj.plocation}</td>
                <td><a href="deptView?dno=${proj.dnum.dnumber}">${proj.dnum.dname}</a></td>
            </tr>
        </c:forEach>
    </table>
    ...
    <table>
        ...
        <th>First Name</th>
        <th>Hours</th>
        <c:forEach var = "emp" items="${projEmployee}">
            <tr>
                <td><a href="empView?ssn=${emp.employee.ssn}">${emp.employee.ssn}</a></td>
                <td>${emp.employee.lname}</td>
                <td>${emp.employee.fname}</td>
                <td>${emp.hours}</td
            </tr>
        </c:forEach>
    </table>